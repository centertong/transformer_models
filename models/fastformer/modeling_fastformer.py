...
class FastformerAttentionLayer(nn.Module):
    def __init__(self, config):
        ...
        self.wq = nn.Parameter(torch.rand(
            self.num_attention_heads, self.attention_head_size))
        self.wk = nn.Parameter(torch.rand(
            self.num_attention_heads, self.attention_head_size))
    ...
    def forward(
        ...
        # B, H, L, C
        query_weight = torch.einsum(
            'bhlc, hc->bhl', query_layer, self.wq) * (self.attention_head_size ** -0.5)
        ...
        query_weight = torch.exp(
            query_weight - torch.amax(query_weight, dim=-1, keepdim=True))
        ...
        key_weight = torch.einsum(
            'bhlc, hc->bhl', query_key, self.wk) * (self.attention_head_size ** -0.5)

        if attention_mask is not None:
            key_weight = key_weight + ~attention_mask_tmp * -1e15

        key_weight = torch.exp(
            key_weight - torch.amax(key_weight, dim=-1, keepdim=True))
...
class FastformerEncoder(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.config = config
        self.layer = nn.ModuleList([FastformerLayer(config)
