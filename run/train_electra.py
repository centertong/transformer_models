...
from dataset.electra import ElectraDataset, ElectraWrapper
from utils import trainer

import argparse

class ElectraTrainer(trainer.Trainer):
    def __init__(self, model, tokenizer, optimizer, scheduler, device=None,
                 ...
                 log_dir='./logs'):
        super().__init__(model, tokenizer, optimizer, scheduler, device, train_batch_size, test_batch_size, checkpoint_path, model_name)
...
def main():
    ...
    dataset = ElectraDataset(tokenizer, args.max_len, path=args.train_file)


    model = ElectraWrapper(getModel(args.config), getModel(args.config))
    ...
    scheduler = torch.optim.lr_scheduler.StepLR(optimizer,  # Optimzer
                                              step_size=int(len(dataset) * 0.9) // args.batch_size,  # Gamma 비율로 줄일 스텝사이즈
                                              gamma=0.9)
        
    trainer = ElectraTrainer(model, tokenizer, optimizer, scheduler, model_name=args.name, device=args.device, checkpoint_path=args.save_path, train_batch_size=args.batch_size, test_batch_size=args.batch_size * 2)
